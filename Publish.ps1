...
# Progress messages go to the information stream, so they can be captured or silenced by the caller.
...
if ($runExitCode -ne 0) {
	Write-Information ""
	Write-Information "The release run did not succeed: https://github.com/$repoFullName/actions/runs/$runId"
	...
	if ($LASTEXITCODE -eq 0 -and $jobId) {
		...
		if ($LASTEXITCODE -eq 0 -and $annotation) {
			Write-Information "Reason: $annotation"
		}
	}

	Write-Information ""
	Write-Information "Tag $version is pushed but no package was published. Once the cause is fixed:"
